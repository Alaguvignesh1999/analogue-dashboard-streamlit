...
import { getEventLineStyle, segmentedControlStyle, THEME_FONTS, themeDashPattern, themeStrokeWidth } from '@/theme/chart';
import {
...
export function DecayTab() {
  ...
  const eventColorMap = useMemo(() => {
    ...
    events.forEach((event, index) => {
      map[event.name] = getEventLineStyle(event.name, index).color;
    });
    return map;
  }, [events]);
            ...
            <div className="flex">
              {(['live-sim', 'all-available'] as const).map((mode) => (
                <button
                  ...
                  style={segmentedControlStyle(scoringMode === mode)}
                >
            ...
            <div className="h-[280px] px-4">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={scoreData} margin={{ top: 10, right: 30, bottom: 5, left: 10 }}>
                  ...
                  <XAxis dataKey="offset" stroke={CHART_THEME.textMuted} tick={{ fontSize: 9, fill: CHART_THEME.textMuted, fontFamily: THEME_FONTS.mono }} tickFormatter={(value) => `D+${value}`} />
                  <YAxis domain={[0, 1.05]} stroke={CHART_THEME.textMuted} tick={{ fontSize: 9, fill: CHART_THEME.textMuted, fontFamily: THEME_FONTS.mono }} />
                  <Tooltip contentStyle={{ background: CHART_THEME.bgCell, border: `1px solid ${CHART_THEME.gridBright}`, borderRadius: 0, fontSize: 10, fontFamily: THEME_FONTS.mono }} labelFormatter={(value) => `Day+${value}`} />
                  ...
                  {segments.map((segment, index) => (
                    <ReferenceArea
                      ...
                      x1={segment.start}
                      x2={segment.end}
                      fill={eventColorMap[segment.event] || CHART_THEME.textDim}
                      fillOpacity={0.08}
                    />
                  ))}

                  <ReferenceLine x={dayN} stroke={CHART_THEME.live} strokeDasharray={themeDashPattern('3 3')} strokeWidth={themeStrokeWidth(1.5)} />

                  {eventNames.map((eventName, index) => {
                    const lineStyle = getEventLineStyle(eventName, index, 1.5);
                    return (
                      <Line key={eventName} dataKey={eventName} stroke={lineStyle.color} strokeWidth={lineStyle.strokeWidth} strokeDasharray={lineStyle.strokeDasharray} dot={false} connectNulls={false} />
                    );
                  })}
                </LineChart>
              </ResponsiveContainer>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={rankData} margin={{ top: 10, right: 30, bottom: 5, left: 10 }}>
                  ...
                  <YAxis
                    ...
                    tick={{ fontSize: 9, fill: CHART_THEME.textMuted, fontFamily: THEME_FONTS.mono }}
                  ...
                  {eventNames.map((eventName, index) => {
                    const lineStyle = getEventLineStyle(eventName, index, 1.5);
                    return (
                      <Line key={eventName} dataKey={eventName} stroke={lineStyle.color} strokeWidth={lineStyle.strokeWidth} strokeDasharray={lineStyle.strokeDasharray} dot={{ r: 2 }} connectNulls={false} />
                    );
                  })}
                </LineChart>
              </ResponsiveContainer>
                ...
                <div className="flex flex-wrap gap-2">
                  {latestScores.slice(0, 5).map((score, index) => (
                    <div key={score.event} className="flex items-center gap-1.5 px-2 py-1 bg-bg-cell border border-border/50">
                      ...
                      <span className="text-2xs text-accent-blue">{score.score.toFixed(3)}</span>
