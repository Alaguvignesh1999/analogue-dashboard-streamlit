    ...
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      <ChartCard title="Trade Proposal" subtitle={`Directional sizing from live D+${displayDay} to D+${displayEndDay}`}>
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={row.asset} className="border-b border-border/20 hover:bg-bg-cell/40 transition-colors">
                  ...
                  <td className="px-3 py-2 text-accent-blue font-medium">{row.rr.toFixed(2)}x</td>
                  <td className="px-3 py-2 text-text-secondary">{(row.hitRate * 100).toFixed(0)}%</td>
                  <td className="px-3 py-2 text-text-primary font-semibold">{row.kellyPct.toFixed(1)}%</td>
        ...
        <BottomDescription className="space-y-2">
          ...
          <div>
            Half-Kelly sizing formula: `f = (p*b - q) / b`, scaled by {(KELLY_FRACTION * 100).toFixed(0)}% and capped to the working budget of <span className="text-accent-blue">{fmtDollar(RISK_BUDGET_USD)}</span>.
          </div>
        </BottomDescription>
      </ChartCard>
    </div>
