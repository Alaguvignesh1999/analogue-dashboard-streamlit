...
function crowdingAccent(overlapCount: number): string {
  if (overlapCount === 0) return alphaThemeColor('textMuted', '0.45');
  if (overlapCount === 1) return alphaThemeColor('accentAmber', '0.9');
  return alphaThemeColor('down', '0.9');
}
...
export function ScreenerTab() {
  ...
  const rows = useMemo(() => {
    ...
    for (const label of labels) {
      ...
      let convictionColor = alphaThemeColor('bgCell', '0.7');
      let rationale = 'Coverage or hit rate is too weak for action.';

      if (bimodal) {
        conviction = 'SPLIT';
        convictionColor = alphaThemeColor('down', '0.12');
        ...
        if (hitRate >= 0.75 && cov >= 0.7) {
          conviction = 'ACT';
          convictionColor = alphaThemeColor('up', '0.18');
          rationale = 'High directional agreement with decent coverage.';
        } else {
          conviction = 'MONITOR';
          convictionColor = alphaThemeColor('accentAmber', '0.15');
            ...
            {rows.length === 0 ? (
              ...
              >
                <td
                  ...
                  style={{ backgroundColor: index % 2 === 0 ? alphaThemeColor('bgHover', '0.22') : 'transparent' }}
                >
