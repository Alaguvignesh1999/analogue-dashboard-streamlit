        ...
        {metrics.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-4 gap-2">
              <StatBox label="Positive" value={stats.bullish} sub="signal > 0" color={CHART_THEME.up} />
              <StatBox label="Negative" value={stats.bearish} sub="signal < 0" color={CHART_THEME.down} />
              <StatBox label="Avg Consistency" value={stats.avgConsistency.toFixed(0)} sub="%" color={CHART_THEME.accentAmber} />
              <StatBox label="Avg Coverage" value={`${(stats.avgCoverage * 100).toFixed(0)}%`} sub="of events" color={CHART_THEME.accentTeal} />
            </div>
            ...
            <div className="overflow-x-auto border border-border/40 rounded-sm">
              <table className="w-full text-2xs font-mono">
                ...
                <tbody>
                  {metrics.map((metric) => {
                    const returnColor = metric.signalScore > 0 ? 'text-up' : metric.signalScore < 0 ? 'text-down' : 'text-text-muted';
